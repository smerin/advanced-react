...
const Mutations = {
  ...
  async createOrder(parent, args, ctx, info) {
    ...
    const user = await ctx.db.query.user(
      ...
      `{
        ...
        cart {
          id
          quantity
          item { title price description image largeImage }
        }
      }`
    );
    ...
    const charge = await stripe.charges.create({
      ...
      currency: "USD",
      source: args.token
    }); // you can pass a description / id here
    ...
    const order = await ctx.db.mutation.createOrder({
      data: {
        total: charge.amount,
        charge: charge.id,
        items: { create: orderItems }, // creates orderItems automatically 👍
        user: { connect: { id: userId } }
      }
    }); // Put catch here to thrown our own error, not in the frontend alert
